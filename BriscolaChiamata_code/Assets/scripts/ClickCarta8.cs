...
public class ClickCarta8: MonoBehaviour
{
    ...
    public void ConfermaCarte()
    {
        ...
        if (Partita.cartachiamata==default(Carta)) {
            ...
            if (ok) {
                ...
                else if (CarteSottrazione.Count == 32)
                {
                    ...
                    /*print("Le carte del bot sono: \n");

                    foreach (Carta x in Mazzo)
                    {
                        print(x.figura + " " + x.seme + "\n");
        ...
        {
            //CODICE DA ESEGUIRE SE LA PARTITA E'GIA'INIZIATA, FA SCEGLIERE SOLO UNA CARTA 

            /*Partita.cartaselezionata = CarteSottrazione.Carte[CarteSottrazione.Carte.Count-1];
            print(Partita.cartaselezionata.figura+ " di "+ Partita.cartaselezionata.seme);
            SceneManager.LoadScene("TurnoGioco", LoadSceneMode.Additive);*/
